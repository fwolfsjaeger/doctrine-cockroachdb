 ...
 */
final class BasicEntityPersister implements EntityPersister
{
    use LockSqlHelper;
     ...
     * The quote strategy.
     */
    protected \Doctrine\ORM\Mapping\DefaultQuoteStrategy|QuoteStrategy $quoteStrategy;
     ...
     * {@inheritDoc}
     */
    public function getResultSetMapping(): Query\ResultSetMapping
    {
     ...
     * {@inheritDoc}
     */
    public function getOwningTable($fieldName): string
    {
     ...
     * @return string
     */
    protected function getSelectColumnAssociationSQL(string $field, $assoc, ClassMetadata $class, string $alias = 'r'): string
    {
        if (! ($assoc['isOwningSide'] && $assoc['type'] & ClassMetadata::TO_ONE)) {
            return '';
        }
    ...
    public function getInsertSQL(): ?string
    {
        ...
        if (empty($columns)) {
            $identityColumn = $this->quoteStrategy
                ->getColumnName($this->class->identifier[0], $this->class, $this->platform);
        ...
        if ($this->platform instanceof CockroachDBPlatform) {
            $insertPostfix = $this->platform->getInsertPostfix($this->class);
        }

        $this->insertSql = sprintf(
     ...
     * {@inheritDoc}
     */
    public function getSelectConditionStatementSQL($field, $value, $assoc = null, $comparison = null): string
    {
        $selectedColumns = [];
     ...
     * @return string
     */
    protected function getSelectConditionSQL(array $criteria, $assoc = null): string
    {
        $conditions = [];
