...
final readonly class CockroachDBMetadataProvider implements MetadataProvider
{
     ...
     * @see https://www.postgresql.org/docs/current/catalog-pg-collation.html
     * @see https://www.postgresql.org/docs/current/catalog-pg-depend.html
     * @see https://www.postgresql.org/docs/current/catalog-pg-description.html
     * @see https://www.postgresql.org/docs/current/catalog-pg-inherits.html
     *
    ...
    private function createTableColumn(array $row): TableColumnMetadataRow
    {
        ...
        if (null !== $domainTypeName && !$this->platform->hasDoctrineTypeMappingFor($typeName)) {
        ...
        $fixed = false;
        $autoincrement = 'd' === $identity;

        if (
            is_string($defaultExpression)
            && preg_match("/^(?:nextval\('([^']+)'(::.*)?\)|unique_rowid\(\))$/", $defaultExpression) === 1
        ) {
            $autoincrement = true;
        }
        ...
        if (null !== $collationName && 'default' !== $collationName) {
    ...
    private function buildTableQueryPredicate(
        ...
        assert((null === $tableName) === (null === $schemaName));
    ...
    private function buildNamespaceNamePredicate(string $columnName): string
    {
        return sprintf(
            "
                %1\$s NOT LIKE 'pg\_%%'
                AND %1\$s NOT LIKE 'crdb\_%%'
                AND %1\$s != 'information_schema'
            ",
            $columnName,
        );
    }
